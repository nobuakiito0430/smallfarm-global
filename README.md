...
- **📚 最新リサーチ**: AI分析済みの最新論文・ニュース
- **🤖 毎日自動更新**: GitHub Actions + Gemini APIで新着記事を自動分析

### 差別化ポイント
- AIが毎日自動で新着記事・論文を分析し、構造化データとして抽出
...
GitHub Actions (毎朝6時JST)
  └── collect.py → OpenAlex API + RSSフィード
  └── analyze.py → Gemini API で構造化分析
...
| `OPENALEX_EMAIL` | OpenAlex ポライトプール用メール | 任意のメールアドレス |
...
- **AI分析**: [Google Gemini API](https://ai.google.dev/) (無料枠)
- **データ収集**: Python + [OpenAlex API](https://openalex.org/) + RSS
